...
def main():
    ...
    # 获取schema, table_name,  rows,size, business_term,id,type
    sql_get_row = f"""
    ...
    if len(values) > 0:
        ...
        # 根据主键id进行upsert, rows或size有变化则更新rows, size, update_time
        insert_table_origin = """
        ...
        on conflict(id)
        do update set rows=EXCLUDED.rows,size=EXCLUDED.size,update_time=EXCLUDED.update_time
        where table_origin.rows!=EXCLUDED.rows or table_origin.size!=EXCLUDED.size
        ;
        """
        ...
        for database, values_oid_list in values_oid_dir.items():
            ...
            for i in values_oid_list:
                ...
                for i in result3:
                    ...
                    index += 1
            conn4.close()
        # 根据id进行upsert，fieldtype,field_length变化时更新fieldtype, field_length, update_time
        insert_field_origin = """
        ...
        on conflict(id)
        do update set fieldtype=EXCLUDED.fieldtype,field_length=EXCLUDED.field_length,update_time=EXCLUDED.update_time
        where field_origin.fieldtype!=EXCLUDED.fieldtype or field_origin.field_length!=EXCLUDED.field_length
        ;
        """
